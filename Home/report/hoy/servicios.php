...
$serviciosHoy = Atencionservicio::all(); // Aquí debes filtrar las vacunas por la fecha de hoy
$serviciosHoy = array_filter($serviciosHoy, function($serviciosHoy) {
    return $serviciosHoy->fecha_servicio == date('Y-m-d');
});
...
<head>
    ...
    <title>Reporte de Servicios brindados - Día</title>
    <style>
        body {
...
<body>
    <header>
        ...
        <h2>Servicios brindados de ' . ucfirst(strftime('%A, %d de %B de %Y')) . '</h2>
    </header>
    <main>
        <table>
            <thead>
...
$contador = 1;
foreach ($serviciosHoy as $serviciosHoy) {
    $perfil = Perfil::find($serviciosHoy->id_personal); // Obtener información del personal
$servicios = Servicios::find($serviciosHoy->id_servicio);

    // Buscar la mascota relacionada con la desparasitacion$serviciosHoy
    $mascota = Mascotas::find($serviciosHoy->id_mascota);
    $propietario = Propietarios::find($mascota->id_propietario); 
    
    // Generar la fila de la tabla con los detalles de la desparasitacion$serviciosHoy
    $html .= '<tr>
                <td>' . $contador++ . '</td>
                <td>' . $mascota->nombre . '</td>
                <td>' . $propietario->nombres . ' ' . $propietario->apellido_paterno . ' ' . $propietario->apellido_materno . '</td>
                <td>' . $serviciosHoy->observaciones . '</td>
                <td>' . $servicios->nombre_servicio . '</td>
                <td>' . number_format($serviciosHoy->costo, 2, '.', ',') . ' Bs.</td>   
